...
def run_vertex_ai_inference(local_input_path, local_output_path):
    """Uploads input, runs Vertex AI, downloads segmentation mask to output_path."""
    # --- GCS Bucket Configuration ---
    INPUT_BUCKET = "nnunet-input-bucket"
    OUTPUT_BUCKET = "nnunet-output-bucket"
    PROJECT_ID = 'gemma-hcls25par-722'
    REGION = 'europe-west1'
    ENDPOINT_ID = '8337460333983563776'
    GCS_INPUT_BLOB_NAME = f"tests/{os.path.basename(local_input_path)}"
    GCS_OUTPUT_PREFIX = "test-results/"
    # Upload input
    upload_to_gcs(local_input_path, f"gs://{INPUT_BUCKET}/{GCS_INPUT_BLOB_NAME}")
    # Run prediction
    ...
    if predictions and len(predictions) > 0:
        ...
        if result.get("status") == "success" and "output_gcs_uris" in result:
            for gcs_output_uri in result["output_gcs_uris"]:
                if gcs_output_uri.endswith('.nii.gz'):
                    download_from_gcs(gcs_output_uri, local_output_path)
                    return
            raise Exception("No .nii.gz output found in output_gcs_uris.")
        else:
